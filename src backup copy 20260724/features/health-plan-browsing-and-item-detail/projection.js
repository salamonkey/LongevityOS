...
function toDefinitionFromGeneratedItem(generatedItem) {
  ...
  const displayName = String(generatedItem?.name ?? '').trim() || 'Preventive item';
  const cadenceText = String(generatedItem?.cadenceLabel ?? '').trim() || 'By recommendation';
  const whyItMattersText = String(generatedItem?.whyItMatters ?? '').trim() || 'Why this matters is not available yet.';
  ...
  return {
    ...
    cadenceText,
    recommendationText: whyItMattersText,
    whyItMattersText,
  };
}

    ...
    <main className={appShellClassName}>
      <section className="app-panel sl001-shell">
        ...
        <footer className="sl001-shell-footer">
          <p className="sl001-kicker">Health OS v.{APP_VERSION}</p>
        </footer>
      </section>
    </main>
  );
...
export function PrioritySection({
  ...
  return (
    <section className={`sl001-priority-section ${sectionClass}`} aria-labelledby={headingId}>
      {canToggle ? (
        <button
          type="button"
          className="sl001-priority-section-toggle"
          aria-expanded={expanded}
          aria-controls={contentId}
          onClick={onToggle}
        >
          <span className="sl001-priority-section-header-text">
            <span id={headingId} className="sl001-priority-section-heading">{title}</span>
            <span className="sl001-priority-toggle-chevron" aria-hidden="true">{expanded ? '▼' : '▶'}</span>
          </span>

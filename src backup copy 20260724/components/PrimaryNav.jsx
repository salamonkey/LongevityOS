...
export default function PrimaryNav({
  ...
  const showProfileSwitcher = Array.isArray(profiles) && profiles.length > 1 && typeof onProfileChange === 'function';
    ...
    <>
      <nav className={`desktop-top-nav${navLocked ? ' is-locked' : ''}`} aria-label="Primary navigation">
        <div className="desktop-top-nav-inner">
          {showProfileSwitcher ? (
            <label className="desktop-top-nav-profile-switcher">
              ...
              >
                {profiles.map((profile) => (
                  <option key={`desktop-profile-${profile.profileId}`} value={profile.profileId}>
                    {profile.displayLabel || profile.name || 'Profile'}
                  </option>
                ))}
              </select>
            </label>
      ...
      <nav className={`mobile-bottom-nav${navLocked ? ' is-locked' : ''}`} aria-label="Primary navigation">
        {showProfileSwitcher ? (
          <label className="mobile-nav-profile-switcher">
            <span>Family member</span>
            <select
              value={activeProfileId ?? ''}
              onChange={(event) => onProfileChange(event.target.value)}
            ...
            >
              {profiles.map((profile) => (
                <option key={`mobile-profile-${profile.profileId}`} value={profile.profileId}>
                  {profile.displayLabel || profile.name || 'Profile'}
                </option>
              ))}
            </select>

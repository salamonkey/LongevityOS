...
- Prove repeat value through status tracking, reminders, and family visibility.
...
- Display an up-to-date summary percentage based only on health-action statuses.
...
- Let one account manage multiple family profiles with separate plans, summaries, and vaccinations.
...
- Use clear, calm, trustworthy language with light motivation.
...
- Use German as the default MVP product language.
...
- Default MVP delivery is a responsive web app optimized for mobile screens.

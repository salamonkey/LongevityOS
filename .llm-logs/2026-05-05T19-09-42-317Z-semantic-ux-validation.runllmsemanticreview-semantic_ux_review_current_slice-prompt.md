...
- provider: openai
- model: gpt-5.4
- started_utc: 2026-05-05T19:09:42.317Z
- prompt_chars: 99441
- prompt_estimated_tokens: 24861
- prompt_sources:
...
export default function App() {
  ...
  const handleProfileNavigate = (target = {}) => {
    ...
    if (target?.planSnapshot) {
      setRuntimePlanSnapshot(target.planSnapshot);
    }
...
export default function FamilyOnboardingAndFamilyOverview({
  ...
  const activeDashboardProjection = useMemo(() => {
    ...
    return buildDashboardProjectionForSlice(activePlanSnapshot, {
      name: activeProfile.displayLabel |
[truncated]
```
## src/features/family-onboarding-and-family-overview/index.js
```text
...
function countOutstandingByBucket(items, bucket) {
  return items.filter((item) => item.dashboardBucket === bucket && item.status !== PLAN_STATUSES.done).length;
}
  ...
  return (
    <AppShell title={getCategoryLabel(activeCategory)}>
      {typeof onNavigate === 'function' ? (
        <button type="button" className="sl002-back-to-dashboard" onClick={handleB
[truncated]
```
## src/features/health-plan-browsing-and-item-detail/definitions.js
```text
...
export const PROFILE_AREA_REMINDER_TIMING_LABELS = Object.freeze({
  [REMINDER_TIMING_TYPES.one_month]: '1 month before',
  [REMINDER_TIMING_TYPES.three_months]: '3 months before',
});

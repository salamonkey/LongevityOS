...
- provider: openai
- model: gpt-5.4
- started_utc: 2026-05-05T11:59:49.014Z
- prompt_chars: 99593
...
export default function VaccinationTrackingAreaAndManualEntries({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setConfirmation('Vaccination entry saved and added to your records.');
      closeAddFlow();
    } catch {
      setPending(false);
...
function toAllowedVaccinationKeys(planSnapshot) {
  ...
  const keys = readModel.vaccinations.map((item) => item.itemKey);

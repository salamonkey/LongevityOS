...
- provider: openai
- model: gpt-5.4
- started_utc: 2026-05-04T07:55:11.821Z
- prompt_chars: 101341
- prompt_estimated_tokens: 25336
- prompt_sources:
...
export function HealthScoreCard({ score, summary }) {
  ...
  const safeScore = Number.isFinite(parsedScore) ? Math.max(0, Math.round(parsedScore)) : 0;
...
export function ReminderSelector({
  ...
  return (
    <section className="panel reminder-sheet" aria-label="Reminder timing">
      ...
      <div className="actions">
        <button type="button" className="primary" disabled={isSaving} onClick={onSave}>
...
export function resolveStatusSemantics(domainStatus) {
  ...
  const key = DOMAIN_STATUS_TO_KEY[normalized] ?? 'due';
...
export function resolvePrioritySemantics(domainPriority) {
  ...
  const key = DOMAIN_PRIORITY_TO_KEY[normalized] ?? 'later';

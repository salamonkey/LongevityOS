...
- provider: openai
- model: gpt-5.4
- started_utc: 2026-05-05T12:06:04.990Z
- prompt_chars: 99593
- prompt_estimated_tokens: 24899
- prompt_sources:
...
export default function SelfOnboardingToFirstDashboard({
  ...
  const openVaccinations = () => {
    if (typeof onOpenVaccinations !== 'function' || !planSnapshot || !profile) return;
    onOpenVaccinations({ planSnapshot, profile });
  };
  ...
  if (isDashboardReady && projection) {
    ...
    if (typeof onOpenHealthPlan === 'function' || typeof onOpenVaccinations === 'function') {
      dashboardHeaderAction = (
        <div className="sl001-dashboard-actions">
          {typeof onOpenHealthPlan === 'function' ? (
            <button type="button" className="sl001-primary-action sl001-dashboard-plan-cta" onClick={openHealthPlan}>
              Browse full plan
            </button>
          ) : null}
          {typeof onOpenVaccinations === 'function' ? (
            <button type="button" className="sl001-secondary-action sl001-dashboard-vaccination-cta" onClick={openVaccinations}>
              Vaccinations
            </button>
          ) : null}
        </div>
